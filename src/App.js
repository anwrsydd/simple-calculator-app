  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center">
        ...
        <div className="bg-gray-300 w-3/4 md:w-fit p-2 space-y-2 rounded-lg drop-shadow-2xl">
            ...
            <input className="bg-gray-200 text-lg p-1 rounded-md w-full text-right" type="text" id="result" readonly value={val} />
            <div className="grid grid-cols-4 gap-3">
                {keyList.map((k, i) => (
                    <button className={`bg-gray-200 p-2 md:p-3 rounded-xl ${(isNaN(k) && k !== ".") ? "text-red-500" : ""} font-medium`} onClick={() => clicked(k)} key={k}>{k}</button>
                ))}
            </div>
        </div>
    </div>
